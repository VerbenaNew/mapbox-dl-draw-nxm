...
module.exports = function (ctx) {
  ...
  events.mousemove = function (event) {
    ...
    // ctx.store.delete("snapedid");
    // ctx.store.delete("snapbbox");
    // const snapNode = snapVertex(event, ctx);
    // event.snapNode = snapNode;
  ...
  events.mouseup = function (event) {
    const target = getFeaturesAndSetCursor(event, ctx);
    // event.featureTarget = target[0];
    // event.featureTarget2 = target;
  ...
  events.keydown = function (event) {
    ...
    var cur_mode = ctx.api.getMode(); //wzy添加,实现不显示删除按钮但可以删节点
    if ((event.keyCode === 8 || event.keyCode === 46) && (ctx.options.controls.trash || cur_mode == "direct_select")) {
  ...
  function changeMode(modename, nextModeOptions, eventOptions = {}) {
    //如果不是切换到面切割，则fire事件
    if (modename != 'polygon_clip' && modename != 'create_building') {
      ctx.map.fire('noDrawFlyLand')
    }
    //如果不是选择模式则off掉click
    if (modename != 'simple_select' && ctx.options.clickFunction) {
    ...
    if (!isLineClip) {
      ...
      if (selectedFeatures.length == 0) {
        ctx.options.openMessage('没有选中要素,不可拆分');
        return
      }
    }
  ...
  const api = {
    ...
    changeVertex: function () {
      currentMode.changeVertex();
    },
    addVertex: function () {
      currentMode.addVertex();
    },
    saveBounds: function () {
      currentMode.SaveBounds();
    },
    getMode: function () {
      return currentModeName;
    }
