...
SimpleSelect.onTap = SimpleSelect.onClick = function (state, e) {
  //如果选中面则fire事件featureSelect
  if (CommonSelectors.isFeature(e) && (e.featureTarget._geometry.type == 'Polygon' || e.featureTarget._geometry.type == 'MultiPolygon')) {
...
SimpleSelect.clickOnVertex = function (state, e) {
  ...
  target2.forEach(fea => {
    if (fea.properties.meta === "vertex") {
      featureIds.push(fea.properties.parent);
...
SimpleSelect.onCombineFeatures = function () {
  ...
  var multiFeature=selectedFeatures[0].toGeoJSON();
  for(var i=1;i<selectedFeatures.length;i++){
    var feature=selectedFeatures[i].toGeoJSON();
    multiFeature=union(multiFeature,feature)
  }
  multiFeature.properties=selectedFeatures[0].properties;
  // var isInteract=intersect(helpers.polygon(selectedFeatures[0].getCoordinates()), helpers.polygon(selectedFeatures[1].getCoordinates()));
