...
const Store = module.exports = function (ctx) {
  ...
  this.undo_stack=[];//添加undo
  this.redo_stack=[];//添加redo
  this.changedIds=[];//变化要素的id,因为store中取不到
  this.ctx = ctx;
  this.sources = {
    hot: [],
    cold: []
...
//undo进栈
Store.prototype.undoPush = function (feature) {
  this.undo_stack.push(feature)
};
//undo出栈
Store.prototype.undoPop = function (feature) {
  this.undo_stack.pop()
};
//redo进栈
Store.prototype.redoPush = function (feature) {
  this.undo_stack.push(feature)
};
//redo出栈
Store.prototype.redoPop = function () {
  this.undo_stack.pop()
};
Store.prototype.undo=function(){
  
  var del_ids=this.changedIds;
  if(this.undo_stack.length==0) return
  var fea=this.undo_stack.pop();
  var del_arr=[];
  var len1=del_ids.length;
  var len2 = fea.length;
  // if (len === 0) return;
  for (var i = 0; i < len1; i++) {
    var ff=this.get(del_ids[i]);
    del_arr.push(ff);
    this.delete(del_ids[i]);
  }
  this.changedIds=[];
  for (var i = 0; i < len2; i++) {
    this.add(fea[i]);
    this.changedIds.push(fea[i].id)
  }
  this.redo_stack.push(del_arr);
}
Store.prototype.redo=function(){
  if(this.redo_stack.length==0) return
  var fea=this.redo_stack.pop();
  var del_ids=this.changedIds;
  var del_arr=[];
  var len1=del_ids.length;
  var len2 = fea.length;
